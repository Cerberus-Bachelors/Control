...
ODriveUSB::~ODriveUSB()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("ODrive"), "USB1...");
...
int ODriveUSB::init(const std::vector<std::vector<int64_t>> & serial_numbers)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Init USB...");
  ...
  RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Init USB2...");
  for (ssize_t i = 0; i < device_count; ++i) {
    libusb_device * device = device_list[i];
    libusb_device_descriptor descriptor;

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Device count: %d/%d", i, device_count);
    
    if (libusb_get_device_descriptor(device, &descriptor) != LIBUSB_SUCCESS) {
      RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Skipped");
      
      continue;
    }

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try vendor");
    
    if (
      descriptor.idVendor == ODRIVE_USB_VENDORID && descriptor.idProduct == ODRIVE_USB_PRODUCTID) {
      libusb_device_handle * device_handle;

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try open");
      if (libusb_open(device, &device_handle) != LIBUSB_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Failed to open");
    
        continue;
      }

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try kernel");
      if (
        (libusb_kernel_driver_active(device_handle, 2) != LIBUSB_SUCCESS) &&
        (libusb_detach_kernel_driver(device_handle, 2) != LIBUSB_SUCCESS)) {
          RCLCPP_INFO(rclcpp::get_logger("ODrive"), "kernel not active");
    
        libusb_close(device_handle);
        continue;
      }

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try cliam");
      if ((libusb_claim_interface(device_handle, 2)) != LIBUSB_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Claimed");
        libusb_close(device_handle);
        continue;
      }

    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try serial");
      uint64_t serial_number;
      if ((read(device_handle, SERIAL_NUMBER, serial_number)) != LIBUSB_SUCCESS) {
        RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Read");
    ...
    RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Try insert");
      odrive_map_.insert(std::pair<int64_t, libusb_device_handle *>(-serial_number, device_handle));
    }
  }


  RCLCPP_INFO(rclcpp::get_logger("ODrive"), "Init USB3...");
